...
export type ContactFormContentInput = z.infer<typeof ContactFormContentInputSchema>;

const ContactFormContentOutputSchema = z.object({
  isAppropriate: z.boolean().describe('Whether the content is appropriate for a business inquiry.'),
  isRelevant: z.boolean().describe('Whether the content is relevant to the website or business.'),
  reason: z.string().optional().describe('The reason why the content might be inappropriate or irrelevant.'),
});
export type ContactFormContentOutput = z.infer<typeof ContactFormContentOutputSchema>;
...
const prompt = `You are an AI assistant specializing in analyzing contact form submissions to ensure they are appropriate and relevant for a business inquiry.
  ...
  Provide your analysis in the following JSON format:
  {
    ...
    "reason": "[reason if inappropriate or irrelevant]"
  }
...
const analyzeContentFlow: AIFunction<typeof ContactFormContentInputSchema, typeof ContactFormContentOutputSchema> = defineFlow(
  {
  ...
  async (input) => {
    const llmResponse = await model.generate({
      ...
      output: {
        format: 'json',
        schema: ContactFormContentOutputSchema,
      },
    });
